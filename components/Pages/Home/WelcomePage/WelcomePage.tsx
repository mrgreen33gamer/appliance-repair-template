  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        if (p.flake) {
          ctx.strokeStyle = '#c026d3'; ctx.lineWidth = 0.6;
          ctx.translate(p.x, p.y);
          for (let i = 0; i < 6; i++) { ctx.rotate(Math.PI / 3); ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, p.r * 3.2); ctx.stroke(); }
        } else { ctx.fillStyle = '#c026d3'; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fill(); }
        ctx.restore();
        p.x += p.vx; p.y += p.vy;
  ...
  return (
    <div className={styles.thermo} aria-hidden="true">
      ...
      <div className={styles.thermoLabels}>
        <span className={styles.thermoTop}>Fixed</span>
        <span className={styles.thermoMid}>Waco, TX</span>
        <span className={styles.thermoBot}>Broken</span>
      </div>
    </div>
  );
}

const CHIPS = ['Same-Day Service', 'All Major Brands', 'Factory-Trained', '16+ Yrs Local', '90-Day Warranty'];
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.content}>
          <motion.div className={styles.badge}
            ...
            Waco&apos;s Most Trusted Appliance Repair — Since 2010
          </motion.div>

          <motion.h1 className={styles.headline}
            initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 0.1 }}>
            Broken Appliance?<br />We Fix It Fast.<br />
            <span className={styles.accentLine}>FixFirst.</span>
          </motion.h1>

          <motion.p className={styles.sub}
            initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.55, delay: 0.22 }}>
            Same-day service available. Upfront pricing. 90-Day Repair Warranty.
            Serving Waco and Central Texas with factory-trained techs.
          </motion.p>

          <motion.div className={styles.ctaRow}
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5, delay: 0.34 }}>
            <a href="tel:+12547908800" className={styles.ctaPrimary}>
